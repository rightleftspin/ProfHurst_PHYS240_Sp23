...
for istep in range(nstep):
    ...
    #if istep >= 2:
    #    time_last_three = t_plot[istep - 2:istep + 1]
    #    theta_last_three = th_plot[istep - 2:istep + 1]
    #    intrpf_partial = partial(intrpf, x = time_last_three, y = theta_last_three)
    #    intrpf_vectorized = np.vectorize(intrpf_partial)
    #    theta_interpolated = intrpf_vectorized(np.linspace(time_last_three[0], time_last_three[1], num = 5))
