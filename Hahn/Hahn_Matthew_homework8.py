# -*- coding: utf-8 -*-
"""
Created on Thu Apr 27 13:32:43 2023
...
method = eval(input('Choose a numerical method: 1) FTCS; 2) Lax; 3)Lax-Wendroff; 4)Upwind Scheme: '))
...
if method == 1:  ### FTCS Method ###
    ...
    plotlabel = 'Upwind scheme'
else:
    raise ValueError('Incorrect index chosen for method. Must choose 1, 2, 3, or 4')
...
for iStep in range(nStep):  ## MAIN LOOP ##
    ...
    elif method == 4:  ### Upwind scheme ###
        a[:] = tau*(-c*(a[:]-a[im])/(x[:] - x[im])) + a[:]
    else:
        raise ValueError('Incorrect index chosen for method. Must choose 1, 2, 3, or 4')
